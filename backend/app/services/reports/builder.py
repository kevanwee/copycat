...
def build_report_payload(
    case, artifacts, media_type, similarity, legal_nodes, outcome, rulepack
):
    ...
    if media_type == "video":
        ...
        dependencies["ffmpeg"] = subprocess.run(
        ...
        dependencies["ffprobe"] = subprocess.run(
            [settings.ffprobe_bin, "-version"],
            capture_output=True,
            text=True,
            check=True,
            timeout=10,
    ...
    content = dict(
        ...
        legal_flow=[asdict(n) for n in legal_nodes],
        assessment=outcome,
        scoring_version=settings.scoring_version,
        rule_pack_version=rulepack["version"],
        rule_pack_id=rulepack["rule_pack_id"],
        rule_pack_sha256=fingerprint(rulepack),
        dependencies=dependencies,
    )
    report = dict(
        content,
        report_id=fingerprint(content),
        generated_at=datetime.now(UTC).isoformat(),
        case_id=case.id,
        headline_overlap_percentage=round(similarity["headline_score"] * 100, 2),
        headline_score=similarity["headline_score"],
        component_scores=similarity["component_scores"],
        evidence=similarity.get("evidence", {}),
        citations=rulepack["citations"],
        legal_reviewed_on=rulepack["reviewed_on"],
        source_status=rulepack["source_status"],
        disclaimers=[
            "Triage aid. User assessments and evidence references have not been independently verified.",
