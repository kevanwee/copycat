...
class RequestLimits:
    ...
    async def __call__(self, scope, receive, send):
        ...
        limit = (max(settings.max_text_mb, settings.max_image_mb, settings.max_video_mb) * 1024 * 1024 if upload else 0) + 256 * 1024
        if upload:
            try:
                with SessionLocal() as db:
                    require_case(db, path.split('/')[4], headers.get(b'x-case-token', b'').decode('ascii', errors='ignore'))
        ...
        if declared > limit:
            return await JSONResponse({'detail': 'Request exceeds the server upload limit'}, status_code=413)(scope, receive, send)
        ...
        if exceeded:
            await JSONResponse({'detail': 'Request exceeds the server upload limit'}, status_code=413)(scope, receive, send)
