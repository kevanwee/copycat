...
def upload_pair(
    client,
    case,
    content=b"An original explanation of copyright and creative expression in Singapore.",
    media="text",
    extension="txt",
):
    for role in ("original", "alleged"):
        response = client.post(
            f"/api/v1/cases/{case}/artifacts",
            data={"role": role, "media_type": media},
            files={"file": (role + "." + extension, content)},
        )
        assert response.status_code == 200, response.text
...
def test_access_is_case_scoped(client):
    ...
    for url in [
        f"/api/v1/cases/{a}",
        f"/api/v1/cases/{a}/report",
        f"/api/v1/cases/{a}/report.pdf",
        f"/api/v1/jobs/{job}",
    ]:
...
def test_reproducible_reports_and_revision_invalidation(client):
    ...
    response = client.put(
        f"/api/v1/cases/{case}/intake", json={"title": "Revised <b>title</b>"}
    )
    assert response.status_code == 200
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("binary.txt", b"\xff\x00"),
        ("spaces.txt", b"  "),
        ("script.html", b"<h1>test</h1>"),
    ],
)
...
def test_replacement_and_deletion_remove_files(client):
    ...
    with SessionLocal() as db:
        paths = [
            Path(a.storage_path) for a in db.query(Artifact).filter_by(case_id=case)
        ]
...
def test_expiry_covers_report_text_and_derived_files(client):
    ...
    (derived / "test.jpg").write_bytes(b"derived")
    with SessionLocal() as db:
...
def test_repetition_coverage_and_excerpt_containment():
    ...
    assert result.coverage["original"] < 0.01
...
def test_image_flow_and_preview(client):
    ...
    assert (
        client.get(f"/api/v1/cases/{case}/preview/original").headers["content-type"]
        == "image/png"
    )
    from app.core.config import get_settings

    derived = get_settings().report_root / case
    assert derived.exists()
    assert client.put(f"/api/v1/cases/{case}/intake", json={}).status_code == 200
    assert not derived.exists()
    assert client.get(f"/api/v1/cases/{case}/preview/original").status_code == 409
...
def test_chunked_body_limit(client):
    ...
    response = client.post(
        "/api/v1/cases", content=body, headers={"Content-Type": "application/json"}
    )
