...
def complete_intake():
    answers = {
        q["id"]: {
            "answer": "yes" if q["id"] in PACK["core_requirements"] else "no",
            "basis": "Recorded evidence, exhibit 1",
        }
        for q in PACK["questions"]
    }
    return dict(
        work_category="literary", conduct_date="2026-09-01", assessments=answers
    )
...
def test_fair_use_is_not_a_checkbox_discount():
    ...
    assert outcome(facts)["status"] == "incomplete"
    facts["fair_use_factors"] = {
        k: "Factor evidence" for k in ("purpose", "nature", "amount", "market")
    }
    assert outcome(facts)["status"] == "review_required"
    facts["fair_use_factors"].update(
        context="criticism_review",
        acknowledgment="impossible",
        acknowledgment_basis="Cannot locate author",
    )
    assert outcome(facts)["status"] == "incomplete"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"claim_route": "secondary"},
        {"claim_route": "authorisation"},
        {"conduct_date": "2019-01-01"},
        {"conduct_date": "2024-01-01"},
        {"conduct_date": "2027-01-01"},
    ],
)
...
def test_strict_assessments():
    for facts in [
        ...
        {"assessments": {"substantial_part": {"answer": "yes"}}},
    ]:
